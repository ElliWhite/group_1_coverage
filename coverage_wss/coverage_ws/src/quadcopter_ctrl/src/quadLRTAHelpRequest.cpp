...
{
    cout << "Callback request for help" << endl;
...
int main(int argc, char **argv)
{
    ...
    BoostGraphWrapper wrapper(file_path);
    ...
    while (ros::ok())
    {
        if ((myLRTA.isCompleted() == false || inSubPath == 1) && (controlSignal.data == -1))
        {
                ...
                else
                {
                    if (inSubPath == 0)
                    {
                        ...
                        else if (dist < threshold)
                        {
                            ...
                            std::cout << "Finding next node for coverage task:" << endl;
                            myLRTA.findNext();
                            updateTarget(updateCount_pub);
        ...
        else if (controlSignal.data != -1 && help_request_received)
        {
            if (quadPosAcquired)
            {
                quadPosAcquired = 0;
                ...
                {
                    path.clear();
                    wrapper.computeShortestPath(myLRTA.getCurrentIndex(), controlSignal.data, path);
                    wrapper.printPath(path);
                    loaded = 1;
                    inSubPath = 0;
                    wpIndex = 0;
                    ...
                    int i = myLRTA.getCurrentIndex() / wrapper.gridSizeY;
                    int j = myLRTA.getCurrentIndex() % wrapper.gridSizeY;
                    targetPos.pose.position.x = static_cast<float> (i) * MAP_SCALE - OFS_X;
                    targetPos.pose.position.y = static_cast<float> (j) * MAP_SCALE - OFS_Y;
                    targetPos.pose.position.z = zHeight;
                }
                ...
                if (inSubPath == 0)
                {
                    ...
                    else if (dist < threshold)
                    {
                        ...
                        else
                        {
                            ...
                            int i = path[wpIndex - 1] / wrapper.gridSizeY;
                            int j = path[wpIndex - 1] % wrapper.gridSizeY;
                            targetPos.pose.position.x = static_cast<float> (i) * MAP_SCALE - OFS_X;
                            targetPos.pose.position.y = static_cast<float> (j) * MAP_SCALE - OFS_Y;
                            targetPos.pose.position.z = zHeight;
            ...
            { /// THIS PART IS EXECUTED IF VREP IS NOT RUNNING
                if (running == 1)
                {
                    printf("%s[%s] ** No incoming vrep/quadcopPos! (waiting...) **%s\n", TC_YELLOW, argv[1], TC_NONE);
                }
