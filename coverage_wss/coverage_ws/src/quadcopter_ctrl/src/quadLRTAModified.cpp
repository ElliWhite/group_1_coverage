...
#include "PathPlanningAlgModified.h"
#include "LRTAstar.h"
#include <fstream>
#include <vector>
...
void quadPosFromVrep(const geometry_msgs::PoseStamped::ConstPtr& pubQuadPose)
{
    ...
    quadPosAcquired = 1;
    //std::cout << "Position of the robot ACQUIRED!" << std::endl;
}
...
int main(int argc, char **argv)
{
    ...
    // BoostGraphWrapper wrapper(filename);
    // std::vector<int> path;
    // wrapper.computeShortestPath(3, 139, path);
    ...
    if (PathPlanningAlgModified::LoadParams(controlMode, MAP_SCALE, OFS_X, OFS_Y, WP_STEP, CRIT_DIST, threshold))
    {
...
void updateTarget(ros::Publisher& countPub)
{
    ...
    /*
    std::cout << "void updateTarget" << std::endl;
...
void publishSubTarget(ros::Publisher& posPub)
{
    ...
    PathPlanningAlgModified::InterpNewPoint(&quadPos, &targetPos, dSubWP);
